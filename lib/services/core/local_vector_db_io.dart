...
class LocalVectorDatabase {
  ...
  Future<void> saveToDisk() async {
    ...
    if (kIsWeb) {
      // Для веба не сохраняем
      print('Save to disk not implemented for web');
      return;
    } else {
      // Для mobile и desktop используем файл
      final dir = await getApplicationDocumentsDirectory();
      final file = File('${dir.path}/vector_db.json');
  ...
  Future<void> _loadFromDisk() async {
    if (kIsWeb) {
      ...
      try {
        ...
        print('Error loading vector database from file: $e');
      }
    }
  }
